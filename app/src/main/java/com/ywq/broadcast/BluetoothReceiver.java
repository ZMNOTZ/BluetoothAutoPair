...
public class BluetoothReceiver extends BroadcastReceiver{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(BluetoothDevice.ACTION_FOUND.equals(action)){  //发现设备
			Log.e("found device:", "["+btDevice.getName()+"]"+":"+btDevice.getAddress());
			
			if(!TextUtils.isEmpty(btDevice.getName()) && btDevice.getName().contains("Printe00"))//HC-05设备如果有多个，第一个搜到的那个会被尝试。
			{//DC:0D:30:22:49:7B
		...
		}else if(action.equals("android.bluetooth.device.action.PAIRING_REQUEST")) //再次得到的action，会等于PAIRING_REQUEST
		{
			...
			if(btDevice.getName().contains("Printe00"))
			{
				Log.e("here", "OKOKOK");
